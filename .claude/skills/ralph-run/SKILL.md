...
if [ ! -f prd.json ]; then
  ...
  echo "  or"
  echo "  /ralph-convert tasks/prd-[name].md"
  exit 1
fi
...
echo "✓ Found $REMAINING incomplete stories"
```

### 2. Verify PROJECT_SPEC.md exists (recommended)
...
The script:
1. Runs Claude with RALPH prompt
2. Each iteration implements ONE story
3. Checks for `<promise>COMPLETE</promise>` signal
